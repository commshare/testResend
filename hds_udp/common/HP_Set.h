#ifndef _HIGH_PERFORMANCE_SET_H_
#define _HIGH_PERFORMANCE_SET_H_
...
此set对于多线程读是安全的
如果多线程写需要加入同步机制。
HP_set与stl的set特性一致，插入相同key的数据，旧数据不会被覆盖。
*/
...
public:
	HP_set():Pool_node(sizeof(rb_node))
	{
		root.rb_node=0;
		root.size=0;
	}
	...
	inline int  insert(const T1 &first)
	{
